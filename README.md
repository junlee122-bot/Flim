...
- 메인: **오늘의 추천 영화** + 씨네필 **큐레이션** 리스트
- **영화 검색** (제목 → 포스터/원제/연도)
- **영화 상세**: 기본정보 · 포스터/스틸컷 · TMDb/IMDb/Metacritic/RT 평점 · 수상 경력 · 평론가 코멘트
...
- 추천 기준: 현재 관리자 지정. 평점/장르/수상 기반 자동 추천으로 확장 가능.
- 평론가: `critics` 테이블 / `src/lib/critics.ts` 에 추가 (박평식·김혜리·정성일 등).
- KOFIC API 연동(한국영화/박스오피스 보강)은 `src/lib/` 에 모듈 추가로 확장.
